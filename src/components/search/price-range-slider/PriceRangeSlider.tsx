...
import { usePageContext, useSettings } from "../ProductsProvider";
...
const PriceRangeSlider = () => {
  ...
  const klevuPriceFilter = filters.find((filter) => filter.key === "klevu_price") as any;
  const klevuFilterMin = Number(klevuPriceFilter?.min) || DEFAULT_MIN_VAL;
  const klevuFilterMax = Number(klevuPriceFilter?.max) || DEFAULT_MAX_VAL;
  ...
  useEffect(() => {
    setRange([klevuFilterMin, klevuFilterMax]);
  }, [klevuFilterMin, klevuFilterMax])
  ...
  return (
    <div className="my-4 flex flex-col gap-4">
      ...
      <Slider
        range={true}
        min={Number(klevuPriceFilter?.min) || DEFAULT_MIN_VAL}
        max={Number(klevuPriceFilter?.max) || DEFAULT_MAX_VAL}
        value={range}
